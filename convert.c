...
/*	{ "ASCII",		 "ascii"	  }, built_in */
	{"CHAR", "pg_catalog.chr($*)" },
	{"CONCAT", "pg_catalog.concat($1::text, $2::text)" },
/*	{ "DIFFERENCE", "difference" }, how to ? */
	...
	{"LCASE", "pg_catalog.lower($*)" },
/*	{"LEFT",		 "left"		  }, built_in */
	{"%2LOCATE", "pg_catalog.strpos($2,  $1)" },	/* 2 parameters */
	{"%3LOCATE", "pg_catalog.strpos(substring($2 from $3), $1) operator(pg_catalog.+) $3 operator(pg_catalog.-) 1" },	/* 3 parameters */
	{"LENGTH", "pg_catalog.char_length($*)"},
/*	{ "LTRIM",		 "ltrim"	  }, built_in */
/*	{"RIGHT",		 "right"	  }, built_in */
	{"SPACE", "pg_catalog.repeat(' ', $1)" },
/*	{ "REPEAT",		 "repeat"	  }, built_in */
/*	{ "REPLACE", "replace" }, ??? */
/*	{ "RTRIM",		 "rtrim"	  }, built_in */
/*	{ "SOUNDEX", "soundex" }, how to ? */
	{"SUBSTRING", "pg_catalog.substr($*)" },
	{"UCASE", "pg_catalog.upper($*)" },
...
/*	{ "FLOOR",		 "floor" 	  }, built_in */
	{"LOG", "pg_catalog.ln($*)" },
	{"LOG10", "pg_catalog.log($*)" },
/*	{ "MOD",		 "mod" 		  }, built_in */
/*	{ "PI",			 "pi" 		  }, built_in */
/*	{"POWER", 		 "power"	  }, built_in */
/*	{ "RADIANS",		 "radians"	  }, built_in */
	{"%0RAND", "pg_catalog.random()" },	/* 0 parameters */
	{"%1RAND", "(pg_catalog.setseed($1) operator(pg_catalog.*) .0 operator(pg_catalog.+) pg_catalog.random())" },	/* 1 parameters */
...
/*	{ "TAN",		 "tan"		  }, built_in */
	...
	{"DAYNAME",	 "pg_catalog.to_char($1, 'Day')" },
	...
	{"MONTHNAME",	 " pg_catalog.to_char($1, 'Month')" },
/*	{ "NOW",		 "now"		  }, built_in */
	...
	// TIMESTAMPADD()
	{"TIMESTAMPADD(SQL_TSI_YEAR", "($3 operator(pg_catalog.+) pg_catalog.make_interval(years := $2))" },
	{"TIMESTAMPADD(SQL_TSI_MONTH", "($3 operator(pg_catalog.+) pg_catalog.make_interval(months := $2))" },
	{"TIMESTAMPADD(SQL_TSI_WEEK", "($3 operator(pg_catalog.+) pg_catalog.make_interval(weeks := $2))" },
	{"TIMESTAMPADD(SQL_TSI_DAY", "($3 operator(pg_catalog.+) pg_catalog.make_interval(days := $2))" },
	{"TIMESTAMPADD(SQL_TSI_HOUR", "($3 operator(pg_catalog.+) pg_catalog.make_interval(hours := $2))" },
	{"TIMESTAMPADD(SQL_TSI_MINUTE", "($3 operator(pg_catalog.+) pg_catalog.make_interval(mins := $2))" },
	{"TIMESTAMPADD(SQL_TSI_SECOND", "($3 operator(pg_catalog.+) pg_catalog.make_interval(secs := $2))" },
	{"TIMESTAMPADD(SQL_TSI_FRAC_SECOND", "($3 operator(pg_catalog.+) pg_catalog.make_interval(secs := $2::float operator(pg_catalog./) 1000000))" },
	// TIMESTAMPDIFF()
...
*/
	...
	*/
	...
	{"TIMESTAMPDIFF(SQL_TSI_FRAC_SECOND", "pg_catalog.mod(cast(extract(second from ($3 operator(pg_catalog.-) $2)) as numeric), 1.0) operator(pg_catalog.*) 1000000" },
	...
	const SQLLEN cbValueMax, const ConnectionClass * const conn)
{
	...
	if (0 == bytea_process_kind)
	{
		if (get_convtype() > 0) /* conversion between the current locale is available */
		{
			...
			MYLOG(MIN_LOG_LEVEL, "localize=%d hybrid=%d is_utf8=%d same_encoding=%d wcs_debug=%d\n", localize_needed, hybrid, is_utf8, same_encoding, wcs_debug);
		}
	}
	if (fCType == SQL_C_WCHAR)
	{
		...
		{
			MYLOG(MIN_LOG_LEVEL, "hybrid estimate\n");
	...
	if (changed || needbuflen > cbValueMax)
	{
		...
		if (fCType == SQL_C_WCHAR)
		{
			...
			else
			{
				...
				{
					MYLOG(MIN_LOG_LEVEL, "hybrid convert\n");
	...
	SQLLEN * const length_return)
{
	...
	MYLOG(MIN_LOG_LEVEL, "field_type=%u type=%d\n", field_type, fCType);
	...
	MYLOG(MIN_LOG_LEVEL, "DEFAULT: len = " FORMAT_LEN ", ptr = '%.*s'\n", len, (int) len, ptr);
...
#ifdef	UNICODE_SUPPORT
	if (SQL_C_WCHAR == fCType)
		MYLOG(MIN_LOG_LEVEL, "    SQL_C_WCHAR, default: len = " FORMAT_LEN ", cbValueMax = " FORMAT_LEN ", rgbValueBindRow = '%s'\n", len, cbValueMax, rgbValueBindRow);
	else
#endif /* UNICODE_SUPPORT */
	if (SQL_C_BINARY == fCType)
		MYLOG(MIN_LOG_LEVEL, "    SQL_C_BINARY, default: len = " FORMAT_LEN ", cbValueMax = " FORMAT_LEN ", rgbValueBindRow = '%.*s'\n", len, cbValueMax, copy_len, rgbValueBindRow);
	else
		MYLOG(MIN_LOG_LEVEL, "    SQL_C_CHAR, default: len = " FORMAT_LEN ", cbValueMax = " FORMAT_LEN ", rgbValueBindRow = '%s'\n", len, cbValueMax, rgbValueBindRow);
		...
		SQLLEN *pcbValue, SQLLEN *pIndicator)
{
	...
	MYLOG(MIN_LOG_LEVEL, "field_type = %d, fctype = %d, value = '%s', cbValueMax=" FORMAT_LEN "\n", field_type, fCType, (value == NULL) ? "<NULL>" : value, cbValueMax);

	if (!value)
	{
MYLOG(MIN_LOG_LEVEL, "null_cvt_date_string=%d\n", conn->connInfo.cvt_null_date_string);
...
MYLOG(DETAIL_LOG_LEVEL, "2stime fr=%d\n", std_time.fr);
	...
	if (fCType == SQL_C_DEFAULT)
	{
		...
		MYLOG(MIN_LOG_LEVEL, ", SQL_C_DEFAULT: fCType = %d\n", fCType);
	}
	...
	if (text_bin_handling)
	{
		...
		switch (field_type)
		{
			...
			case PG_TYPE_UUID:
				...
				midtemp[i] = '\0';
				MYLOG(MIN_LOG_LEVEL, "PG_TYPE_UUID: rgbValueBindRow = '%s'\n", rgbValueBindRow);
				break;
	...
	else
	{
		...
		if (field_type == PG_TYPE_MONEY)
		{
			...
			{
				MYLOG(MIN_LOG_LEVEL, "couldn't convert money type to %d\n", fCType);
...
MYLOG(DETAIL_LOG_LEVEL, "SQL_C_VARBOOKMARK value=%d\n", ival);
				...
				{
					MYLOG(MIN_LOG_LEVEL, "couldn't convert the type %d to SQL_C_BINARY\n", field_type);
					return COPY_UNSUPPORTED_TYPE;
				}
				break;
			case SQL_C_GUID:
				...
				if (COPY_OK != result)
				{
					MYLOG(MIN_LOG_LEVEL, "Could not convert to SQL_C_GUID\n");
					return	COPY_UNSUPPORTED_TYPE;
				}
				len = sizeof(g);
				if (bind_size > 0)
			...
			default:
				MYLOG(MIN_LOG_LEVEL, "conversion to the type %d isn't supported\n", fCType);
				return COPY_UNSUPPORTED_TYPE;
		}
	}
...
#define	return	DONT_CALL_RETURN_FROM_HERE???
	...
	MYLOG(MIN_LOG_LEVEL, "parsed for the first command length=" FORMAT_SSIZE_T "(" FORMAT_SSIZE_T ") num_p=%d\n", endp2, endp1, num_p1);
	...
	while (multi > 0)
	{
		...
		MYLOG(MIN_LOG_LEVEL, "parsed for the subsequent command length=" FORMAT_SSIZE_T "(" FORMAT_SSIZE_T ") num_p=%d\n", endp2, endp1, num_p1);
		pstmt = buildProcessedStmt(srvquery,
								   endp2 < 0 ? SQL_NTS : endp2,
								   fake_params ? 0 : num_p1);
		if (!pstmt)
...
inner_process_tokens(QueryParse *qp, QueryBuild *qb)
{
	...
	if (oldchar == ODBC_ESCAPE_START)
	{
		...
		if (SQL_ERROR == convert_escape(qp, qb))
		{
			...
			MYLOG(MIN_LOG_LEVEL, "convert_escape error\n");
			return SQL_ERROR;
		}
		PT_TOKEN_IGNORE(pt);
		return SQL_SUCCESS;
	...
	else if (oldchar != '?')
	{
		...
		if (pt->token_len > 0)
			MYLOG(MIN_LOG_LEVEL, "token_len=%d status=%x token=%s\n", pt->token_len, pt->in_status, pt->finished_token);
		if (!pt->curchar_processed)
		{
			MYLOG(MIN_LOG_LEVEL, "Forgot to process ParseToken char=%c status=%u\n", oldchar, qp->in_status);
	...
	*nParams = 0;
	if (num_p > 0)
	{
		...
		for (i = 0, pno = 0; i < stmt->num_params; i++)
		{
			...
			if (isbinary)
				MYLOG(MIN_LOG_LEVEL, "%dth parameter is of binary format\n", pno);
...
MYLOG(DETAIL_LOG_LEVEL, "para:%d(%d,%d)\n", param_number, ipdopts->allocated, apdopts->allocated);
	...
	{
		MYLOG(MIN_LOG_LEVEL, "The # of (A|I)PD parameters (%d, %d) < %d the # of parameter markers\n", apdopts->allocated, ipdopts->allocated, param_number);
	...
	*pgType = sqltype_to_bind_pgtype(conn, param_sqltype);
	...
	if (0 == param_sqltype) /* calling SQLSetStmtAttr(.., SQL_ATTR_APP_PARAM_DES, an ARD of another statement) may cause this */
	{
		if (0 != param_pgtype)
		{
			...
			MYLOG(MIN_LOG_LEVEL, "convert from pgtype(%u) to sqltype(%d)\n", param_pgtype, param_sqltype);
		}
	}

	MYLOG(MIN_LOG_LEVEL, "from(fcType)=%d, to(fSqlType)=%d(%u), *pgType=%u\n",
		...
		switch (param_ctype)
		{
			case SQL_C_CHAR:
				if (!same_encoding || wcs_debug)
				{
					...
					MYLOG(MIN_LOG_LEVEL, "locale param convert\n");
			...
			case SQL_C_WCHAR:
				...
				{
					MYLOG(MIN_LOG_LEVEL, "hybrid param convert\n");
...
#ifdef	UNICODE_SUPPORT
		case SQL_C_WCHAR:
MYLOG(MIN_LOG_LEVEL, " C_WCHAR=%d contents=%s(" FORMAT_LEN ")\n", param_ctype, buffer, used);
			if (NULL == send_buf)
			{
		...
		case SQL_C_TYPE_TIMESTAMP:	/* 93 */
			{
				...
				MYLOG(MIN_LOG_LEVEL, "m=%d,d=%d,y=%d,hh=%d,mm=%d,ss=%d\n", st.m, st.d, st.y, st.hh, st.mm, st.ss);
...
MYLOG(MIN_LOG_LEVEL, "cvt_null_date_string=%d pgtype=%d send_buf=%p\n", conn->connInfo.cvt_null_date_string, param_pgtype, send_buf);
	if (conn->connInfo.cvt_null_date_string > 0 &&
	...
	switch (param_sqltype)
	{
		...
		case SQL_LONGVARBINARY:
			...
			if (param_pgtype == PG_TYPE_BYTEA)
			{
				...
				{
					MYLOG(MIN_LOG_LEVEL, "sending binary data leng=" FORMAT_LEN "\n", used);
					*isbinary = TRUE;
				}
				else
				{
					 ...
					 * converted to octal
					 */
					MYLOG(MIN_LOG_LEVEL, "SQL_VARBINARY: about to call convert_to_pgbinary, used = " FORMAT_LEN "\n", used);
		...
		size_t *output_count, SQLLEN param_pos[][2])
{
	...
	{
		MYLOG(MIN_LOG_LEVEL, "closing ) not found %d\n", innerParenthesis);
		qb->errornumber = STMT_EXEC_ERROR;
			...
			if (2 == param_count)
			{
				...
				else if (add_cast)
				{
					...
					if (typel < sizeof(sqltype))
					{
						...
						sqltype[typel] = '\0';
MYLOG(MIN_LOG_LEVEL, FORMAT_LEN "-" FORMAT_LEN " SQLtype=%s SQL_BIT=%d\n", to, from, sqltype, SQL_BIT);
			...
			for (mapptr = mapExpr; *mapptr; mapptr++)
			{
				...
				else if (isdigit((unsigned char) *mapptr))
				{
					    ...
					    param_pos[pidx][0] < 0)
					{
						...
						MYLOG(MIN_LOG_LEVEL, "%dth param not found for the expression %s\n", pidx + 1, mapExpr);
						retval = SQL_ERROR;
						break;
					}
				...
				else
				{
					...
					MYLOG(MIN_LOG_LEVEL, "internal expression error %s\n", mapExpr);
					retval = SQL_ERROR;
					break;
				}
				paramlen = to - from + 1;
...
convert_from_pgbinary(const char *value, char *rgbValue, SQLLEN cbValueMax)
{
	...
	for (i = 0; i < ilen;)
	{
		...
		/** if (rgbValue)
			MYLOG(MIN_LOG_LEVEL, "i=%d, rgbValue[%d] = %d, %c\n", i, o, rgbValue[o], rgbValue[o]); ***/
	}

	if (rgbValue)
		rgbValue[o] = '\0';		/* extra protection */

	MYLOG(MIN_LOG_LEVEL, "in=" FORMAT_SIZE_T ", out = " FORMAT_SIZE_T "\n", ilen, o);
...
convert_to_pgbinary(const char *in, char *out, size_t len, QueryBuild *qb)
{
	...
	MYLOG(MIN_LOG_LEVEL, "leaving " FORMAT_SIZE_T ", out='%.*s'\n", o, (int) o, out);
		   ...
		   SQLLEN cbValueMax, SQLLEN *pcbValue)
{
	...
	if (!gdata_blob || gdata_blob->data_left64 == -1)
	{
		...
		return COPY_NO_DATA_FOUND;
	MYLOG(MIN_LOG_LEVEL, "lo data left = " FORMATI64 "\n", left64);
