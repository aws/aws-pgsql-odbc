...
int main() {
	...
	const char* secret_config = "AUTHTYPE=secrets-manager;SSLMODE=require;REGION=us-east-2;"
		"SECRETID=arn:aws:secretsmanager:us-east-2:123412341234:secret:secret-id-name";

	sprintf(reinterpret_cast<char*>(conn_in), "DSN=%s;SERVER=%s;PORT=%d;DATABASE=%s;%s;", dsn, server, port, db, secret_config);
