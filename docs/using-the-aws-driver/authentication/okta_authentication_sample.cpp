...
int main() {
	...
	sprintf(idp_config, "IDPEndpoint=%s;IDPUserName=%s;IDPPassword=%s;IDPRoleArn=%s;IDPArn=%s;AppId=%s", idp_endpoint, idp_username, idp_password,
			idp_role_arn, idp_arn, app_id);

	sprintf(reinterpret_cast<char*>(conn_in), "DSN=%s;UID=%s;SERVER=%s;PORT=%d;DATABASE=%s;%s;%s", dsn, user, server, port, db, okta_config,
			idp_config);
