...
function Invoke-SignInstaller {
    ...
    $unsignedInstallerPath=$(Join-Path $BuildDir "awspsqlodbc_x64.msi")
    $signedInstallerPath=$(Join-Path $BuildDir "awspsqlodbc-$OdbcVersion-$Architecture-signed.msi")
    ...
    Rename-Item -Path $signedInstallerPath -NewName "aws-pgsql-odbc-$OdbcVersion-$Architecture.msi"
