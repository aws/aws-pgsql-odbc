...
static int utf8_test_one(HSTMT hstmt)
{
	...
	SQLCHAR		str[100];
	SQLCHAR		chardt[100];
	// SQLTCHAR	query[] = _T("select '私は' operator(pg_catalog.||) ?::text operator(pg_catalog.||) 'です。貴方は' operator(pg_catalog.||) ?::text operator(pg_catalog.||) 'さんですね？  𠀋𡈽𡌛𡑮𡢽𪐷𪗱𪘂𪘚𪚲'");
	SQLTCHAR	query[512] = _T("select '私は' operator(pg_catalog.||) ?::text operator(pg_catalog.||) 'です。貴方は' operator(pg_catalog.||) ?::text operator(pg_catalog.||) 'さんですね？  𠀋𡈽𡌛𡑮𡢽𪐷𪗱𪘂'");
